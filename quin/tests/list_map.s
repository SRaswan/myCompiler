...
mov rax, [rbp + 16]         ; load closure into rax
             ;; TODO: check rax is pointer
             sub rax, 5             ; strip tag
             mov rax, [rax + 8*5] ; read at index                    ; read x value into rax
             mov [rbp - 8 *4], rax ; restore to stack
mov rax, [rbp + 16]         ; load closure into rax
             ;; TODO: check rax is pointer
             sub rax, 5             ; strip tag
             mov rax, [rax + 8*6] ; read at index                    ; read x value into rax
             mov [rbp - 8 *5], rax ; restore to stack
           mov rax, [rbp - 8*-4]
                 mov [rbp - 8*6], rax
                 mov rcx, [rbp - 8*6]
             push rcx
...
mov rcx, [rbp - 8*6]
     ...
     ret
         fun_finish_map:
           lea rcx, QWORD [rel fun_start_map]
             mov [r11 + 8*0], rcx
mov rcx, 2
             mov [r11 + 8*1], rcx
mov rcx, [rbp - 8*3]
...
jmp fun_finish_anon_3
     ...
     ret
         fun_finish_anon_3:
           lea rcx, QWORD [rel fun_start_anon_3]
             mov [r11 + 8*0], rcx
mov rcx, 1
             mov [r11 + 8*1], rcx
mov rcx, [rbp - 8*8]
