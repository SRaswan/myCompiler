...
fn check_defn<A: Span>(
    ...
    let expected_ty = _subst.instantiate(_expected);

    let (expected_in_tys, expected_out_ty) = match expected_ty {
        Ty::Fun(ref in_tys, ref out_ty) => (in_tys.clone(), (**out_ty).clone()),
    ...
    let mut bin = vec![];
    if let Some(name) = &_defn.name {
        let poly_fn_ty = generalize(_env, &expected_ty);
        bin.push((name.clone(), poly_fn_ty));
    }
    let mut env = _env.extend(&bin);

    let mut binds = vec![];
    for (x, ty) in _defn.params.iter().zip(&expected_in_tys) {
        binds.push((x.clone(), mono(ty.clone())))
    }
    
    env = env.extend(&binds);

    let body_ty = infer(&env, _subst, &_defn.body)?;

    unify(&_defn.body.ann, _subst, &body_ty, &expected_out_ty.apply(_subst))?;

    Ok(fun(expected_in_tys.clone(), expected_out_ty.clone()).apply(_subst))
...
fn infer_defn<A: Span>(_env: &TypeEnv, _subst: &mut Subst, _defn: &Defn<A>) -> Result<Ty, Error> {
  ...
  let (in_tys, out_ty) = fresh_fun(_defn, _subst);
  let fn_ty = fun(in_tys.clone(), out_ty.clone());
  let mut bin = vec![];
  if let Some(name) = &_defn.name {
    let fun_poly = generalize(_env, &fn_ty);
    bin.push((name.clone(), fun_poly));
  }
  let mut env = _env.extend(&bin);
  // 2. Add the types of the params to the environment
  let mut binds = vec![];
  for (x, ty) in _defn.params.iter().zip(&in_tys) {
    binds.push((x.clone(), mono(ty.clone())))
  }
  env = env.extend(&binds);
  ...
  unify(&_defn.body.ann, _subst, &body_ty, &out_ty.apply(_subst))?;

  // 5. Return the function's template after applying subst
  Ok(fun(in_tys.clone(), out_ty.clone()).apply(_subst))
}

fn fresh_fun<A>(defn: &Defn<A>, subst: &mut Subst) -> (Vec<Ty>, Ty) {
    let in_tys = defn.params.iter().map(|_| subst.fresh()).collect::<Vec<_>>();
    ...
    match op {
        ...
        Prim::ABToVecAB => forall( 
            ...
            Ty::Fun(
                ...
                Box::new(vec(Ty::Var(tv("a")), Ty::Var(tv("b")))),
            ),
        ),
    }
}
...
fn idx_sig(idx: &Index) -> Poly {
    match idx {
        Index:: Zero => forall(
            vec![tv("a"), tv("b")],
            Ty::Fun(
                vec![vec(Ty:: Var(tv("a")), Ty:: Var(tv("b")))],
                Box :: new(Ty :: Var(tv("a"))),
            ),
        ),
        Index:: One => forall(
