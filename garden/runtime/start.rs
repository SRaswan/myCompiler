...
pub unsafe extern "C" fn snek_gc(
    ...
    // println!("BRUHHHHHHHBRUHHHHHHHBRUHHHHHHHBRUHHHHHHHBRUHHHHHHHBRUHHHHHHHBRUHHHHHHH");
    ...
    while ptr >= curr_rsp {
        let mut val = *ptr;
        if val & 0b111 == 0b001 && val >= HEAP_START as u64 {
            val &= !0b111;
            roots.insert(val);
        }
        ptr = ptr.sub(1);
    ...
    let marks = &mut HashSet::new();
    for root in roots.iter() {
        mark((*(root) as *const u64), marks);
    }

    let mut new_heap_ptr = HEAP_START;
    let mut ref_ptr = HEAP_START;

    ptr = HEAP_START;
    let mut obj = HEAP_START;

    let mut new_roots = HashMap::new();
    while ptr < heap_ptr {
        if *ptr & 1 == 1 && ptr >= HEAP_START {
            let len = *(ptr.add(1)) as usize + 2;
            *(ptr as *mut u64) = new_heap_ptr as u64;
            new_roots.insert(ptr as u64 | 0b1, new_heap_ptr as u64 | 0b1);
            new_heap_ptr = new_heap_ptr.add(len);
            // obj = ptr;
            // for i in 0..len-1 {   
            //     if *obj & 0b111 == 0b001 && *obj >= HEAP_START as u64 {
            //         ref_ptr = ref_ptr.add(i);
            //         *(obj as *mut u64) = ref_ptr as u64 | 0b1;
            //         new_roots.insert(obj as *mut u64, ref_ptr as u64);
            //     }
            //     obj = obj.add(1)
            // }
        }
    ...
    // snek_print_heap(heap_ptr);
    ptr = HEAP_START;
    new_heap_ptr = HEAP_START;
    let mut new_ptr = HEAP_START;
    ...
    while ptr < heap_ptr {
        ...
        if *ptr != 0 {
            ...
            let len = *(ptr.add(1)) as usize + 2;
            new_ptr = new_heap_ptr;
            for i in 0..len {
                
                let mut val = *ptr.add(i);
                if val & 0b111 == 0b001 && val >= HEAP_START as u64 {
                    // println!("--- {ptr:?}: {:#0x}", val);

                    match new_roots.get(&(val as u64)) {
                        Some(value) => *(new_ptr as *mut u64) = *value as u64 | 0b1,
                        None => (),
                    }

                } else {
                    *(new_ptr as *mut u64) = *ptr.add(i) as u64;
                }
                new_ptr = new_ptr.add(1);
            }
            *(new_heap_ptr as *mut u64) = 0b0;
            new_heap_ptr = new_heap_ptr.add(len);
        }
        ptr = ptr.add(*(ptr.add(1)) as usize+2);
    }
    ...
    while ptr >= curr_rsp {
        ...
        if val & 0b111 == 0b001 && val >= HEAP_START as u64 {
            match new_roots.get(&(val as u64)) {
                Some(value) => *(ptr as *mut u64) = *value as u64 | 0b1,
                None => (),
            }
        }
        ptr = ptr.sub(1);
...
pub unsafe fn mark(addr: *const u64, marks: &mut HashSet<*const u64>) {
    unsafe {
        if (*addr == 0) {
            marks.insert(addr);
            let gc_word: *mut u64 = addr as *mut u64;
            *gc_word = 1;
            let mut ptr = addr;
