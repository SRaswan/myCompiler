...
fn expr_vars(e: &Expr) -> usize {
  match e {
    ...
    Expr::UnOp(_, e) | Expr::Set(_, e) | Expr::Loop(e) | Expr::Break(e) | Expr::Length(e) => 1+expr_vars(e),
    Expr::Call(_, e1) => match e1 {
      None => 0,
      Some(es) => es.iter().enumerate().map(|(i, e)| 1+i + expr_vars(e)).max().unwrap(),
    },
    Expr::Let(e1, e2) => e1.iter().enumerate().map(|(i, e)| i + expr_vars(&e.1)).max().unwrap() + expr_vars(e2),
    ...
    Expr::Vec(es) => es.iter().enumerate().map(|(i, e)| 1+i + expr_vars(e)).max().unwrap(),
    Expr::Get(e1, e2) => max(expr_vars(e1), 1 + expr_vars(e2)),
...
{exit}:\n")
  ...
  {loop_exit}:\n"
    ...
    Expr::Call(func, e1) => {
      ...
      } else { panic!("Invalid: Function not found {func}"); }
      if tr && func_ext.eq(f) {
        format!("{args_code}\n{push_args}\njmp fun_body_{func}\n")
    ...
    Expr::Vec(exprs) => {
      ...
      let mut heapadd: String = String::new();
      let heapsize = exprs.len()+1;
      save.push_str(&format!("\nmov rax, {heapsize}\nmov [rbp - 8*{sp}], rax\n"));
      heapadd.push_str(&format!("\nmov rax, [rbp - 8*{sp}]\nmov [r12], rax\n")); 

      for (i, expr) in exprs.iter().enumerate() {
        let adj_i = i+1;
        let pos = sp+adj_i;
        let val = compile_expr(expr, env, pos, count, brk, false, &f, inloop, &funcs);
        save.push_str(&format!("\n{val}\nmov [rbp - 8*{pos}], rax\n"));
        heapadd.push_str(&format!("\nmov rax, [rbp - 8*{pos}]\nmov [r12 + 8*{adj_i}], rax\n")); 
      }

      save.push_str(&heapadd);
      save.push_str(&format!("\nmov rax, r12
  add rax, 0x1
  add r12, 8*{}\n", heapsize+1));
      save
    ...
    Expr::Get(e1, e2) => {
      let e1_code = compile_expr(e1, env, sp, count, brk, false, &f, inloop, &funcs);
      let e2_code = compile_expr(e2, env, sp + 1, count, brk, false, &f, inloop, &funcs);
      let testnum = test_number(77);
      let testptr = test_pointer(88);
      let testbound = test_bounds(99);
      format!("\n{e1_code}
  {testptr}
  mov r13, rax
  sub r13, 0x1
...
fn test_pointer(code: usize) -> String {
  format!("\n  mov rcx, rax
  and rcx, 0b111
  cmp rcx, 0b001
  mov rdi, {code}
...
fn test_bounds(code: usize) -> String {
  format!("  mov rcx, rax
  cmp rcx, [r13]
  mov rdi, {code}
  jge label_error
