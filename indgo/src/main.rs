...
fn compile_expr(e: &Expr, env: &Alloc, count: &mut i32, brk: &str, dst: &Loc) -> String {
    match e {
        Expr::Num(_) | Expr::True | Expr::False | Expr::Var(_) | Expr::Input => {
            compile_imm_into(e, env, dst)
        }
        Expr::Add1(_i) => {
            compile_op1(_i, env, dst, |reg| {format!("add {reg}, 2")})
        },
        Expr::Sub1(_i) => {
            compile_op1(_i, env, dst, |reg| {format!("sub {reg}, 2")})

        }
        Expr::Neg(_i) => {
            compile_op1(_i, env, dst, |reg| {format!("neg {reg}")})
        }
        ...
        Expr::Plus(_i1, _i2) => {
            let arg2 = imm_to_arg(_i2, env);
            compile_op1(_i1, env, dst, |reg| {format!("add {reg}, {arg2}")})

        }
        Expr::Mult(_i1, _i2) => {
            let arg2 = imm_to_arg(_i2, env);
            compile_op1(_i1, env, dst, |reg| {format!("\nsar {reg}, 1\nimul {reg}, {arg2}\n")})
        ...
        Expr::Let(_x, _e1, _e2) => {
            ...
            let x_loc = env.lookup_var(_x);
            let e1_code = compile_expr(_e1, env, count, brk, &x_loc);
            let e2_code = compile_expr(_e2, env, count, brk, dst);
            format!("\n{e1_code}\n{e2_code}\n")
        }
