...
fn live(
    ...
    match e {
        ...
        Expr::Let(x, e1, e2) => {
            let out_e2 = live(graph, e2, binds, params, &out);
            let out_e2_diff = out_e2.clone().difference(hashset! {x.clone()});
            let out_e1 = live(graph, e1, binds, params, &out_e2_diff);
            graph.add_variable(x);
            for y in &out_e2 {
                graph.add_conflict(x, y);
            }
        ...
        Expr::Loop(e) => {
            let live_e = live(graph, e, binds, params, out);
            live_e.union(out.clone())
        }
        ...
        | Expr::Le(e1, e2)
        | Expr::Vek(e1, e2) => {
            let live2 = live(graph, e2, binds, params, out);
            live(graph, e1, binds, params, &live2)
        }
        Expr::If(e1, e2, e3) => {
            let live_e3 = live(graph, e3, binds, params, out);
    let live_e2 = live(graph, e2, binds, params, out);
    let live_e1 = live(graph, e1, binds, params, &live_e2.union(live_e3));
    live_e1
        }
        Expr::Set(x, e1) => {
            let live_e1 = live(graph, e1, binds, params, out);
    live_e1.union(hashset! {x.clone()})
        }
        Expr::Block(es) => {
            es.iter().rev().fold(out.clone(), |live_out, e| live(graph, e, binds, params, &live_out))
        }
        Expr::Call(f, es) => {
            let mut out = out.clone();
            for e in es.iter().rev() {
                out = live(graph, e, binds, params, &out);
            }
            live_var(f, params, &out)
        }
        Expr::Fun(defn) => out
            .clone()
...
impl ConflictGraph {
    ...
    fn allocate(&self, regs: &[Reg], offset: usize) -> HashMap<String, Loc> {
        ...
        for (var, color) in colors {
            if color < n {
                alloc.insert(var, Loc::Reg(regs[color]));
            } else {
                alloc.insert(var, Loc::Stack(offset as i32 +color as i32- n as i32));
            }
        }
        println!("alloc {:?}", alloc);
